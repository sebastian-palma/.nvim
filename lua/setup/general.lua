...
local function send_eval_request(args)
	...
	if #response > 0 then
		...
		-- print(vim.inspect({ decoded = vim.inspect(decoded), mode = mode }))

		if mode == "append_as_comment" then
			add_virtual_text(decoded["value"], "Comment")
		end
		...
		if mode == "append_to_tab" then
			...
			-- print("existing_buffer_number: ", existing_buffer_number)
			...
			if existing_buffer_number then
				...
				vim.api.nvim_buf_set_lines(existing_buffer_number, line_count, line_count, false, { decoded["value"] })
			else
...
-- lua function map(tbl, fn) local mapped = {} for _, value in ipairs(tbl) do table.insert(mapped, fn(value)) end return mapped end
--
